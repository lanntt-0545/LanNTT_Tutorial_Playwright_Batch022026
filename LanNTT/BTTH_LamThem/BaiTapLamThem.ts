...
const bookTitle : string = "Lập trình TypeScript";
const bookPrice : number = 150000;
const isAvailable : boolean = true;
const discount : string = "10%";
const STORE_NAME: string = "Sun-Asterisk Book Store"
// STORE_NAME = "Lan"
...
*/
const categories : string[] = ["Programming", "Design", "Logic"];
const bookStatus : [ number, string] = [101, "In stock"];
...
const myLevel = BookLevel.INTERMEDIATE;
console.log(`Gia tri cua myLevel = ${myLevel}`);
...
function calculateTotal(price: number, quantity: number): string{
    const hoaDon : string = `Tổng cộng: ${price*quantity} VND`;
    console.log(hoaDon);
    return hoaDon;
}
...
class Book{
    ...
    getInfo(): void{
        if(this.status === true){
            console.log(`Sach: ${this.name} - Gia: ${this.price} - Trang thai: ${BookStatus.INSTOCK}`)
        }
         if(this.status === false){
            console.log(`Sach: ${this.name} - Gia: ${this.price} - Trang thai: ${BookStatus.OUTSTOCK}`)
        }
    }
}
const book1 : Book[] = [
...
let shippingMethod:  "Standard"|"Express"
